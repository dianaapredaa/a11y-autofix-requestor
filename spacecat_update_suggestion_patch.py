#!/usr/bin/env python3
"""
SpaceCat - Update a suggestion's patch content from a file

Fetches a suggestion (view=full), replaces a field inside `data` (default: patchContent)
with the full contents of a local file, then PATCHes the suggestion back via:
...
PATCH body strategy:
  - By default we take the fetched suggestion payload, update `data.<patch-field>`,
    and send that as the PATCH body (after removing obvious server-managed/read-only keys).
  - If the API rejects extra fields (e.g. 400/422), we retry once with a minimal PATCH body:
      {"data": { ...updated data... }}
"""
...
def _build_patch_body_from_payload(*, suggestion: dict, updated_data: dict) -> dict:
    """
    Build a PATCH body using the fetched suggestion payload as a base.
    Remove common read-only/server-managed keys to reduce 4xx risk.
    """
    patch_body = dict(suggestion)  # shallow copy
    patch_body["data"] = updated_data

    # Common read-only/server-managed keys seen in REST payloads.
    # Keep this conservative; if the API still rejects fields, we'll fall back to minimal PATCH.
    for k in [
        "id",
        "siteId",
        "opportunityId",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Update a SpaceCat suggestion's patch field from a local file")
    ...
    parser.add_argument("--patch-file", required=True, help="Path to file whose full contents become the patch content")
    parser.add_argument(
        "--patch-field",
        default="patchContent",
        help="Field name inside suggestion.data to update (default: patchContent)",
    )
    ...
    try:
        new_patch = _read_text_file(args.patch_file)
    except Exception as e:
        _print_error(str(e))
        sys.exit(2)
    ...
    old_patch = data.get(args.patch_field)
    data[args.patch_field] = new_patch

    # Preferred: use the fetched payload as the PATCH body (minus read-only keys),
    # changing only data.<patch_field>.
    ...
    _print_info(f"patch field: data.{args.patch_field}")
    _print_info(f"old length: {len(old_patch) if isinstance(old_patch, str) else 0}")
    _print_info(f"new length: {len(new_patch)}")
    ...
    try:
        ...
        # If the API rejects extra fields, retry with minimal PATCH body.
        status_code = None
        try:
    ...
    _print_success("Updated suggestion patch content successfully.")
